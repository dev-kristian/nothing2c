...
export async function proxy(request: NextRequest) {
  ...
  if (sessionCookie && isProtectedRoute) {
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`[Proxy] Session verification failed for ${pathname} with status ${response.status}`);
      }
    } catch (error) {
      console.error(`[Proxy] Network error fetching session verification API for ${pathname}:`, error);
    }
  ...
  if (isAuthenticated && isAuthRoute && pathname !== '/verify-email' && pathname !== '/auth-action') {
